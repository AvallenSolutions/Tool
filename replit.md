...
- **Refined Product LCA System**: Integrated calculation system using OpenLCA as authoritative source for ingredient and packaging impacts. Water dilution recorded at product level but excluded from water footprint calculations to prevent double-counting with facility-level consumption. Unified `/api/products/:id/refined-lca` endpoint provides comprehensive per-unit and annual impact data with detailed breakdown.
