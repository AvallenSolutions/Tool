...
Extract specific claims and provide detailed analysis following this exact format:

For each environmental claim found, provide:
...
Focus on:
- Vague terms (sustainable, eco-friendly, green, natural)
- Unsubstantiated claims (carbon neutral, climate positive)
- Missing evidence or third-party verification
- Comparative claims without proper baselines
- Specific measurable claims (quote them exactly)

Be precise and quote actual text from the content.`;
  ...
  try {
    ...
    console.error('AI analysis failed, falling back to pattern analysis:', error);
    // Fallback to pattern analysis if AI fails
    return await fallbackPatternAnalysis(content, type);
  }
}

// Fallback pattern analysis function  
async function fallbackPatternAnalysis(content: string, type: string) {
  const issues = [];
  let baseScore = 75;
  
  // Simple pattern matching as fallback
  if (content.toLowerCase().includes('sustainable')) {
    issues.push({
      type: 'warning',
      category: 'Vague Claims',
      claim: 'sustainable',
      description: 'Vague environmental term requires substantiation',
      solution: 'Provide specific metrics or certifications',
      violationRisk: 55,
  ...
  return {
    ...
    status: baseScore >= 75 ? 'compliant' : baseScore >= 50 ? 'warning' : 'non-compliant',
    issues,
    recommendations: ['Replace vague terms with specific claims'],
    analysisDetails: {
      contentType: type,
