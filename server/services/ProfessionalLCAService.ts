...
export class ProfessionalLCAService {
  ...
  private renderProfessionalPDFKitContent(doc: PDFKit.PDFDocument, data: LCAReportData): void {
    console.log('📄 Rendering content with data:', JSON.stringify({
      companyName: data.company?.name,
      productCount: data.products?.length,
      primaryProductName: data.products?.[0]?.name
    }));
    
    const primaryProduct = data.products?.[0] || {};
    const companyName = data.company?.name || 'Company Name Not Available';
    ...
    // TITLE PAGE - Fix positioning
    doc.fontSize(18).fillColor(colors.secondary);
    doc.text('Life Cycle Assessment of', 50, 120, { align: 'center', width: 500 });
    
    doc.fontSize(24).fillColor(colors.primary);
    doc.text(primaryProduct.name || 'Product', 50, 150, { align: 'center', width: 500 });
    
    doc.fontSize(14).fillColor(colors.secondary);
    doc.text(`Produced by ${companyName}`, 50, 200, { align: 'center', width: 500 });
    
    const reportDate = new Date().toLocaleDateString('en-GB');
    doc.fontSize(12).fillColor(colors.text);
    doc.text('Author: Avallen Sustainability Platform', 350, 400);
    doc.text(`Date: ${reportDate}`, 350, 420);
    
    // MAIN FINDINGS PAGE - Fix overlapping
    doc.addPage();
    doc.fontSize(18).fillColor(colors.text);
    doc.text('Main findings', 50, 80, { underline: true });
    
    // Calculate proper per-unit values
    const totalCarbonKg = data.lcaResults.totalCarbonFootprint * 1000; // Convert tonnes to kg
    const perUnitCarbonKg = totalCarbonKg / (primaryProduct.annualProductionVolume || 1);
    const perUnitWaterL = data.lcaResults.totalWaterFootprint / (primaryProduct.annualProductionVolume || 1);
    
    doc.fontSize(11).fillColor(colors.text);
    doc.text(`In this study, an environmental life cycle assessment (LCA) of ${primaryProduct.name} produced by ${companyName} was conducted. The assessment focused on climate impacts represented by the 'Global Warming Potential in the next 100 years (GWP100)'.`, 50, 120, { 
      align: 'justify', 
      width: 500,
      lineGap: 2
    });
    
    doc.text(`The study shows a carbon footprint of ${perUnitCarbonKg.toFixed(3)} kg CO₂-eq per product unit. The packaging materials have the highest contribution to the climate impacts, followed by ingredient production and processing. The water footprint analysis shows ${perUnitWaterL.toFixed(0)} litres of water consumption per product unit.`, 50, 170, { 
      align: 'justify', 
      width: 500,
      lineGap: 2
    });
    
    // KEY RESULTS BOX - Fix positioning
    doc.rect(50, 230, 500, 120);
    doc.fillAndStroke(colors.light, colors.secondary);
    
    doc.fillColor(colors.text).fontSize(12);
    doc.text('Key Environmental Impact Results:', 60, 245);
    
    doc.fontSize(10);
    doc.text(`Carbon Footprint: ${perUnitCarbonKg.toFixed(3)} kg CO₂e per unit`, 60, 270);
    doc.text(`Water Footprint: ${perUnitWaterL.toFixed(1)} L per unit`, 60, 290);
    doc.text(`Annual Production: ${primaryProduct.annualProductionVolume?.toLocaleString() || 'N/A'} units`, 60, 310);
    doc.text(`Total Annual Impact: ${data.lcaResults.totalCarbonFootprint.toFixed(1)} tonnes CO₂e`, 60, 330);
    
    doc.fontSize(11).fillColor(colors.text);
    doc.text('All calculations follow ISO 14040 and ISO 14044 LCA standards, using the latest environmental impact databases including ecoinvent 3.5 and verified supplier data where available.', 50, 370, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    doc.text('1. Introduction', 50, 80);
    
    doc.fontSize(12).fillColor(colors.text);
    doc.text('1.1. Background', 50, 120);
    
    doc.fontSize(11);
    doc.text(`${companyName} is committed to sustainable production practices. This Life Cycle Assessment (LCA) was conducted to quantify the environmental impacts of ${primaryProduct.name} using the most widely accepted methodology for calculation of environmental impacts, standardized in ISO 14040 and ISO 14044.`, 50, 150, { 
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    
    doc.text('According to these standards, there are four phases in an LCA study:', 50, 200, {
      width: 500,
      lineGap: 2
    });
    doc.text('a) Goal and scope definition', 70, 225);
    doc.text('b) Inventory analysis', 70, 245);
    doc.text('c) Impact assessment', 70, 265);
    doc.text('d) Life cycle interpretation.', 70, 285);
    
    doc.fontSize(12).fillColor(colors.text);
    doc.text('1.2. Goal and scope definition', 50, 320);
    
    doc.fontSize(11);
    doc.text(`The goal of this study is to assess the environmental impacts of ${primaryProduct.name}. Results will be used for internal sustainability reporting and stakeholder communication. The scope of the assessment is 'cradle-to-gate', including raw materials extraction and production processes.`, 50, 350, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    doc.text(`The functional unit is defined as: 1 ${functionalUnit} of ${primaryProduct.name}.`, 50, 420, {
      width: 500,
      lineGap: 2
    });
    
    doc.text('The assessment focuses on climate change impact represented by the Global Warming Potential in the next 100 years (GWP100) as defined by the IPCC, supplemented by water consumption analysis.', 50, 450, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    doc.addPage();
    doc.fontSize(16).fillColor(colors.text);
    doc.text('2. Inventory analysis', 50, 80);
    
    doc.fontSize(12).fillColor(colors.text);
    doc.text('2.1. Process description', 50, 120);
    
    doc.fontSize(11);
    doc.text(`The production process of ${primaryProduct.name} includes ingredient sourcing, processing, packaging, and distribution. Raw materials are sourced from verified suppliers and processed according to industry standards.`, 50, 150, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    doc.fontSize(12).fillColor(colors.text);
    doc.text('2.2. Process data', 50, 200);
    ...
    if (primaryProduct.ingredients && primaryProduct.ingredients.length > 0) {
      doc.fontSize(11).fillColor(colors.text);
      doc.text('Ingredient composition:', 50, currentY, { underline: true });
      currentY += 25;
      
      primaryProduct.ingredients.forEach((ingredient: any) => {
        doc.text(`• ${ingredient.name}: ${ingredient.amount} ${ingredient.unit || 'kg'}`, 70, currentY);
        currentY += 20;
      });
      currentY += 10;
    }
    ...
    if (primaryProduct.bottleWeight || primaryProduct.labelWeight) {
      doc.fontSize(11).fillColor(colors.text);
      doc.text('Packaging specifications:', 50, currentY, { underline: true });
      currentY += 25;
      
      if (primaryProduct.bottleWeight) {
        doc.text(`• Bottle: ${primaryProduct.bottleWeight}g (${primaryProduct.bottleMaterial || 'glass'})`, 70, currentY);
        currentY += 20;
      }
      if (primaryProduct.labelWeight) {
        doc.text(`• Label: ${primaryProduct.labelWeight}g (${primaryProduct.labelMaterial || 'paper'})`, 70, currentY);
        currentY += 20;
      }
    }
    ...
    doc.addPage();
    doc.fontSize(16).fillColor(colors.text);
    doc.text('3. Impact assessment / Interpretation', 50, 80);
    
    doc.fontSize(11);
    doc.text('Results of the life cycle impact assessment are shown below:', 50, 120, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    if (data.lcaResults.impactsByCategory && data.lcaResults.impactsByCategory.length > 0) {
      doc.fontSize(12).fillColor(colors.text);
      doc.text('Impact breakdown by category:', 50, currentY, { underline: true });
      currentY += 30;
      
      data.lcaResults.impactsByCategory.forEach((category: any) => {
        doc.fontSize(10);
        doc.text(`${category.category}: ${category.impact} ${category.unit}`, 70, currentY);
        currentY += 20;
      });
      currentY += 20;
    }
    ...
    doc.fontSize(11);
    doc.text('The assessment shows that the primary environmental impacts come from raw material production and packaging. Energy consumption during processing contributes a smaller but significant portion of the total impact.', 50, currentY, {
      align: 'justify',
      width: 500,
      lineGap: 2
    });
    ...
    doc.text(`Calculation date: ${new Date(data.lcaResults.calculationDate).toLocaleDateString()}`, 50, currentY);
    doc.text(`System: ${data.lcaResults.systemName}`, 50, currentY + 15);
    
    // REFERENCES PAGE
    doc.addPage();
    doc.fontSize(16).fillColor(colors.text);
    doc.text('4. References', 50, 80);
    ...
    references.forEach((ref, index) => {
      doc.text(`[${index + 1}] ${ref}`, 50, currentY, {
        align: 'justify',
        width: 500,
        lineGap: 2
      });
    ...
    doc.text(`Report generated by Avallen Sustainability Platform on ${new Date().toLocaleDateString()}`, 50, currentY, {
      align: 'center',
      width: 500
    });
  }
